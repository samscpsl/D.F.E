...
if ($result -eq 'Yes') {
	...
	Write-Output "ПИСССЮЮЮЮН: "
	Write-Output $exe_path
	...
	New-Service -Name "D.F.E" -BinaryPathName $binaryPathName -StartupType Automatic -Description "Обход by Sam"
	
	Write-Output "Запускаем сервис D.F.E"
	Start-Service -Name "D.F.E"
