...
if ($result -eq 'Yes') {
	...
	Write-Output "Распаковка архива..." -ForegroundColor blue
	...
	$binaryPathName = "$exe_path --wf-tcp=80,443 --wf-udp=443,50000-50099 --filter-udp=443 --hostlist=`"$path\$winws_folder\sosalka_list.txt`" --dpi-desync=fake --dpi-desync-udplen-increment=10 --dpi-desync-repeats=6 --dpi-desync-udplen-pattern=0xDEADBEEF --dpi-desync-fake-quic=`"$path\$winws_folder\quic_initial_www_google_com.bin`" --new --filter-udp=50000-50099 --dpi-desync=fake,tamper --dpi-desync-any-protocol --dpi-desync-fake-quic=`"$path\$winws_folder\quic_initial_www_google_com.bin`" --new --filter-tcp=80 --hostlist=`"$path\$winws_folder\sosalka_list.txt`" --dpi-desync=fake,split2 --dpi-desync-autottl=2 --dpi-desync-fooling=md5sig --new --filter-tcp=443 --hostlist=`"$path\$winws_folder\sosalka_list.txt`" --dpi-desync=fake,split2 --dpi-desync-autottl=2 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls=`"$path\$winws_folder\tls_clienthello_www_google_com.bin`""


	New-Service -Name "DFE" -BinaryPathName $exe_path -StartupType Automatic -Description "Обход_by_Sam"


	Write-Output "Запускаем сервис D.F.E"
	Start-Service -Name "DFE"
	
	$result = [System.Windows.Forms.MessageBox]::Show('D.F.E успешно установлен. Чтобы удалить его, запустите скрипт снова. Вам предложат его удалить. Удачи!' + [System.Environment]::NewLine + [System.Environment]::NewLine + "Ухх бляяяяять..........", "D.F.E" , [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
}
if ($result -eq 'No') {
	exit
}