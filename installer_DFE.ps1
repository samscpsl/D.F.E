...
{	
	...
	if (Test-Path $old_folder_path) {
		Write-Output "Удаление устаревших файлов Winws"
...
Write-Host "Загрузчик и инсталлятор сервиса Winws " -ForegroundColor white -nonewline
Write-Host "282" -ForegroundColor red
Write-Host "РУССКИЙ ЛЕС" -ForegroundColor yellow -BackgroundColor darkred
Write-Host ""
...
$result = [System.Windows.Forms.MessageBox]::Show('Внимание!' + [System.Environment]::NewLine + [System.Environment]::NewLine + "Выключите VPN/туннель/прочие приблуды перед установкой скрипта", "D.F.E" , [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
...
if ($winws_service_exists.Length -gt 0) {
	$result = [System.Windows.Forms.MessageBox]::Show('Найден установленный ранее сервис DFE' + [System.Environment]::NewLine + [System.Environment]::NewLine + 'Удалить?' , "D.F.E" , [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Error)
	if ($result -eq 'Yes') {
...
if ($result -eq 'Yes') {
	...
	Write-Output "Распаковывка архива"
