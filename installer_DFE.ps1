...
if ($result -eq 'Yes') {
	...
	# Создаем действия для выполнения cmd файла
	$action = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c `"$exe_path`""

	# Определяем триггер для задания (при запуске компьютера)
