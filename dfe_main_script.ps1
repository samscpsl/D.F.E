...
if ($gdpi_service_exists.Length -gt 0) {
	$result = [System.Windows.Forms.MessageBox]::Show('У Вас уже установлен DFE' + [System.Environment]::NewLine + [System.Environment]::NewLine + 'Удалить?' , "DFE" , [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Error)
	if ($result -eq 'Yes') {
		Write-Output "Останавливаем и удаляем сервис DFE"
...
$result = [System.Windows.Forms.MessageBox]::Show('Скрипт установит сервис DFE' + [System.Environment]::NewLine + [System.Environment]::NewLine + 'Установить?' , "DFE" , [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
if ($result -eq 'Yes') {
	...
	Write-Output "Устанавка сервиса DFE"
	...
	Write-Output "Запускаем сервис DFE"
	[void](sc.exe start "DFE")
	
	$result = [System.Windows.Forms.MessageBox]::Show('DFE успешно установлен' + [System.Environment]::NewLine + [System.Environment]::NewLine + "Скрипт будет работать всегда с этого момента, пока Вы не удалите его повторным запуском DFE_installer.bat", "DFE" , [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
}
if ($result -eq 'No') {
	exit
}
